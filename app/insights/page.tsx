...
export default function InsightsPage() {
    const { globalStats, rawHistory, currentPeriod, setCurrentPeriod, showPeriodToggle } = useHistory();
    ...
    if (rawHistory.length === 0) {
        return (
            <main className="flex-1 bg-slate-50 flex flex-col items-center justify-center p-8 text-center space-y-4">
                ...
                <p className="text-sm text-slate-500 max-w-sm">Primero debes cargar tus archivos JSON de Spotify en la pantalla de inicio.</p>
                <Link href="/"
            ...
            <div className="max-w-4xl mx-auto space-y-8 pb-12">
                    ...
                    <div>
                        ...
                        {globalStats.timeRange && (
                            <p className="text-sm text-indigo-600 font-bold mt-0.5">
                                📅 Rango seleccionado: <span className="bg-indigo-50 px-2 py-0.5 rounded border border-indigo-100">{globalStats.timeRange.start}</span> al <span className="bg-indigo-50 px-2 py-0.5 rounded border border-indigo-100">{globalStats.timeRange.end}</span>
                            </p>
                        )}
                    </div>

                    {showPeriodToggle && (
                        <div className="bg-slate-200/60 p-1 rounded-xl flex gap-1 text-xs font-bold shadow-inner shrink-0 select-none">
                            <button
                ...
                <div className="grid grid-cols-1 md:grid-cols-5 gap-6 items-stretch">
                    <div className="md:col-span-2 flex flex-col justify-between gap-6">
                        <div className="grid grid-cols-2 gap-4 flex-1">
                            <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm flex flex-col items-center justify-center text-center h-full">
                                ...
                                <div className="text-slate-400 font-bold text-[10px] uppercase tracking-wider mb-1">Minutos Totales</div>
                                <span className="block text-3xl font-black text-indigo-600 tracking-tight">{globalStats.totalMinutes.toLocaleString()}</span>
                            </div>
                            <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm flex flex-col items-center justify-center text-center h-full">
                                ...
                                <div className="text-slate-400 font-bold text-[10px] uppercase tracking-wider mb-1">Canciones Totales</div>
                                <span className="block text-3xl font-black text-indigo-900 tracking-tight">{globalStats.totalTracksCount.toLocaleString()}</span>
                                <span className="text-[10px] font-medium text-slate-400 block mt-0.5">Escuchadas</span>
                            </div>
                        </div>
                            ...
                            <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm flex items-center gap-4 border-collapse">
                                <div className="w-14 h-14 border border-slate-200 text-white rounded-xl shrink-0 flex items-center justify-center text-xl shadow-inner relative overflow-hidden font-bold">
                                    {enrichedTracks[0]?.coverUrl ? (
                                        <img src={enrichedTracks[0].coverUrl} alt="Cover" className="w-full h-full object-cover"/>
                                    ) : "💿"}
                                </div>
                                <div className="truncate space-y-0.5 flex-1">
                                    ...
                                    <p className="text-xs font-semibold text-slate-600 truncate">{globalStats.topSong.artist} • <span className="font-mono font-bold text-slate-800">{globalStats.topSong.plays}</span> plays</p>
                                </div>
                                {enrichedTracks[0]?.spotifyUrl && (
                                    <a href={enrichedTracks[0].spotifyUrl} target="_blank" rel="noreferrer" className="text-xs bg-slate-100 hover:bg-indigo-50 text-slate-700 font-bold p-2 rounded-full shrink-0 transition-colors">▶</a>
                                )}
                            </div>
                        )}
                    </div>
                    ...
                    <div className="md:col-span-3 bg-gradient-to-br from-indigo-950 via-slate-900 to-slate-950 rounded-2xl shadow-xl relative overflow-hidden flex flex-col justify-end p-8 min-h-64 text-white">
                        {topArtist?.imageUrl && (
                            <div className="absolute inset-0 bg-cover bg-center opacity-30 mix-blend-luminosity pointer-events-none" style={{backgroundImage: `url(${topArtist.imageUrl})`}}></div>
                        )}
                        <div className="absolute inset-0 opacity-40 mix-blend-screen pointer-events-none" style={{backgroundImage: "radial-gradient(circle at 50% 10%, rgba(99,102,241,0.4) 0%, transparent 70%)"}}></div>
                        ...
                        {globalStats.topArtists.length > 0 && (
                            <div className="absolute top-6 left-6 space-y-1 z-10">
                                ...
                                <p className="text-xs text-indigo-200/90">Has escuchado a {globalStats.topArtists[0].artist} por más de <span className="font-mono font-bold text-white bg-indigo-500/30 px-1 py-0.5 rounded">{globalStats.topArtists[0].totalMinutes.toLocaleString()}</span> minutos.</p>
                            </div>
                        )}

                        <div className="flex justify-end w-full z-10 mt-24">
                            {topArtist?.spotifyUrl && (
                                <a href={topArtist.spotifyUrl} target="_blank" rel="noreferrer" className="bg-white hover:bg-slate-100 text-slate-950 text-xs font-bold px-5 py-2.5 rounded-full flex items-center gap-1.5 shadow-md transition-all active:scale-95">▶ Ir a Spotify</a>
                            )}
                        </div>
                    </div>
                </div>
                                    ...
                                    <li key={item.artist} className="py-3 flex items-center justify-between gap-3 group first:pt-0 last:pb-0">
                                        <div className="flex items-center gap-3 truncate">
                                            ...
                                            <div className="truncate space-y-0.5">
                                                {spotifyData?.spotifyUrl ? (
                                                    <a href={spotifyData.spotifyUrl} target="_blank" rel="noreferrer" className="font-bold text-slate-900 text-sm truncate block hover:text-indigo-600 hover:underline">{item.artist}</a>
                                                ) : (
                                                    <span className="block font-bold text-slate-900 text-sm truncate">{item.artist}</span>
                                                )}
                                                <span className="block text-[11px] font-medium text-slate-400 truncate">{item.uniqueTracksCount} {item.uniqueTracksCount === 1 ? 'canción diferente' : 'canciones diferentes'}</span>
                                            </div>
                                        </div>
                                        <span className="font-mono text-xs font-bold text-indigo-600 shrink-0 bg-indigo-50/50 px-2 py-1 rounded-md">{item.totalMinutes.toLocaleString()} min</span>
                                    </li>
                                );
                            })}
                        </ol>
                    ...
                    <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm flex flex-col justify-between">
                        ...
                        <ol className="divide-y divide-slate-100 flex-1 flex flex-col justify-around min-h-80">
                            {(enrichedTracks.length > 0 ? enrichedTracks : globalStats.topTracks).map((item, index) => {
                                ...
                                return (
                                    <li key={`${item.track}-${item.artist}`} className="py-3 flex items-center justify-between gap-3 group first:pt-0 last:pb-0">
                                        <div className="flex items-center gap-3 truncate">
                                            <span className="font-mono text-xs font-bold text-slate-300 w-4 group-hover:text-indigo-600 transition-colors">{index + 1}</span>
                                            {trackData?.spotifyUrl ? (
                                                <a href={trackData.spotifyUrl} target="_blank" rel="noreferrer" className="w-10 h-10 bg-indigo-50 text-indigo-500 rounded-lg shrink-0 flex items-center justify-center border border-indigo-100/50 overflow-hidden hover:opacity-80 transition-all">
                                                    {trackData.coverUrl ? <img src={trackData.coverUrl} alt="Cover" className="w-full h-full object-cover"/> : "🎵"}
                                                </a>
                                            ) : (
                                            ...
                                            <div className="truncate space-y-0.5">
                                                {trackData?.spotifyUrl ? (
                                                    <a href={trackData.spotifyUrl} target="_blank" rel="noreferrer" className="font-bold text-slate-900 text-sm truncate block hover:text-indigo-600 hover:underline">{item.track}</a>
                                                ) : (
                                        ...
                                        <span className="font-mono text-xs font-bold text-slate-500 shrink-0">{('plays' in item ? item.plays : (item as any).plays)} plays</span>
                                    </li>
                                );
                            })}
                        </ol>
                ...
                <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm space-y-6">
                    ...
                    <div className="h-48 w-full flex justify-between items-end gap-1.5 border-b border-slate-100 pb-1 pt-6 relative">
                        {globalStats.monthlyDistribution.map((monthData) => {
                            ...
                            return (
                                <div key={`${monthData.year}-${monthData.monthIndex}`} className="flex-1 flex flex-col items-center gap-2 group h-full justify-end relative">
                                  <span className="opacity-0 group-hover:opacity-100 bg-slate-900 text-white text-[10px] font-mono font-bold px-2 py-0.5 rounded bottom-full mb-1 transition-all duration-150 absolute pointer-events-none z-10 shadow-md whitespace-nowrap">
                                    ...
                                    <div style={{height: `${Math.max(4, percentageHeight)}%`}} className={`w-full ${barColorClass} rounded-t-md transition-all cursor-pointer shadow-sm`}></div>
